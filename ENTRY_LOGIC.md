...
```
        ...
        entry ─┤  ← close − InpPWSweepEntryRetrace * (close − open)
                │
        open ──┘
...
CHoCH_BEAR, откат от close вверх к open, стоп над high(CHoCH-свечи) + буфер.
В отличие от пинбара/поглощения, этот сетап **не проверяет** тренд
`InpTrendTF`, ATR-фильтр размера свечи и премиум/дискаунт — сам CHoCH и есть
